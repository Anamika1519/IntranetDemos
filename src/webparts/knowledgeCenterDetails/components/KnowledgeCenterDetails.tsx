...
import "./KnowledgeCenterdetails.scss"
import {
...
import { IKnowledgeCenterDetailsProps } from "./IKnowledgeCenterDetailsProps";
...
const HelloWorldContext = ({ props }: any) => {
  ...
  const callAPI = async () => {
    ...
    debugger
    const getknowledgecenterdata = await fetchARGKnowledgeCenterInsideData(sp, Number(idNum));

    setMediaData(getknowledgecenterdata);

    const catdata = await fetchARGKnowledgeCenterdata(sp);
...
const KnowledgeCenterDetails: React.FC<IKnowledgeCenterDetailsProps> = (props) => (
