...
const NewRequest: React.FC<INewRequestProps> = (props) => {
    ...
    const generateSequenceAndDocNumber = async (index: number, source: 'Project Type' | 'Document Type') => {
        ...
        try {
            ...
            // Create the document number based on details
            const newDocNumber = `${projectInfo.projectName}-${selectedArea ? selectedArea.AreaIdentificationCode : ''
                }-${selectedArea ? selectedArea.AreaCode : ''}-${selectedOrganization ? selectedOrganization.OrganizationIdentificationCode : ''
                }-${selectedDocType ? selectedDocType.DocumentTypeCode : ''}-${newSeqNumber}`;
                    ...
                    items.map(async (item, index) => {
                        ...
                        if (projectInfo.projectName && selectedArea && selectedOrganization && selectedDocType) {
                            try {
                                ...
                                deliverableItem.seqNumber = newSeqNumber;
                                deliverableItem.docNumber = `${projectInfo.projectName}-${selectedArea.AreaIdentificationCode
                                    }-${selectedArea.AreaCode}-${selectedOrganization.OrganizationIdentificationCode
                                    }-${selectedDocType.DocumentTypeCode}-${newSeqNumber}`;
                            } catch (error) {
                                console.error("Error generating sequence for pre-filled deliverables:", error);
                            }
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            console.log('Project request created with ID:', projectCreationListId);
            await addDeliverablesDetails(projectCreationListId);
            await createEmailTriggerDetails();//vishnu added
    ...
    const createEmailTriggerDetails = async () => {
        ...
        //     new Set(deliverables.map(d => d.assignedTo).filter(id => id > 0))
        //   );
        const toUserIds = deliverables
            .map(d => d.assignedTo)
            .filter(id => id > 0);
        ...
        //             Body: `A new task has been assigned to you for the project "${projectInfo.projectName}". Please review the task details and proceed with the required actions.`,
        //             ToUserId: [userId],
        //             CCUserId: [selectedUser.value]
        //         });
        // }
        ...
        var RegardsName = Array.isArray(userOptions)
            ? userOptions.filter(u => String(u.value) === String(selectedUser.value))
            : [];
        var regardsName = RegardsName.length > 0 ? RegardsName[0].label : null;
        for (const user of deliverables) {

            // Build rows for table
            const rows = deliverables.map((d, index) => {
                const no = index + 1;
                ...
                <tr>
                  ...
                  <td style="border:1px solid #ccc;padding:6px;text-align:center;">0</td> <!-- Rev -->
                  <td style="border:1px solid #ccc;padding:6px;text-align:center;">-</td> <!-- Status -->
                  <td style="border:1px solid #ccc;padding:6px;">${transmittal ?? "-"}</td>
                  <td style="border:1px solid #ccc;padding:6px;">${sentDate ?? "-"}</td>
                  <td style="border:1px solid #ccc;padding:6px;">${remarks ?? "-"}</td>
                </tr>
              `;
            }).join("");

            // Build complete email table
            const emailTable = `
            ...
            // Send EmailTrigger entry
            await sp.web.lists.getByTitle("EmailTriggerDetails").items.add({
                Title: projectInfo.projectName,
                Subject: `New Task Assigned – ${projectInfo.projectName}`,
                Body: `
                A new task has been assigned to you for the project "${projectInfo.projectName}".<br/><br/>
                Please review the task details below:<br/><br/>
                ${emailTable}
                <br/><br/>
                Regards,<br/>
                ${regardsName}
              `,
                ToUserId: [user.assignedTo],
                CCUserId: [selectedUser.value]
            });
        }
