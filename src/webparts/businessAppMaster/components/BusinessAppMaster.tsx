...
import { DeleteBusinessAppsAPI, getBusinessApps } from "../../../APISearvice/BusinessAppsService";
import "../components/BusinessApps.scss"
...
import { IBusinessAppMasterProps } from './IBusinessAppMasterProps';
import Provider from '../../../GlobalContext/provider';
import "../../../CustomJSComponents/CustomTable/CustomTable.scss";
import * as XLSX from 'xlsx';
import moment from 'moment';
import HorizontalNavbar from '../../horizontalNavBar/components/HorizontalNavBar';
const BusinessAppscontext = ({ props }: any) => {
  const sp = getSP();
  ...
  const [businessAppsData, setmediaData] = React.useState([]);
  ...
  const ApiCall = async () => {
    const mediaArr = await getBusinessApps(sp);
  ...
  const filteredAnnouncementData = applyFiltersAndSorting(businessAppsData);
  ...
  const Editmedia = (id: any) => {
    ...
    window.location.href = `${siteUrl}/SitePages/BusinessAppsForm.aspx`;
  }
  //#endregion
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          ...
          setmediaData(await getBusinessApps(sp));
        }
        ).catch(async err => {
          setmediaData(await getBusinessApps(sp));
        }
              ...
              <div className="col-lg-9">
                <div className="d-flex flex-wrap align-items-center justify-content-end mt-3">
                  <div className="d-flex flex-wrap align-items-center justify-content-start">
                    ...
                    <a onClick={() => GotoAdd(`${siteUrl}/SitePages/BusinessAppsForm.aspx`)} >
...
const BusinessApps: React.FC<IBusinessAppMasterProps> = (props) => (
  <Provider>
    <BusinessAppscontext props={props} />
  </Provider>
);
