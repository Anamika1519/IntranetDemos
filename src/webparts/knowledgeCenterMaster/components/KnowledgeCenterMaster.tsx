...
import { DeleteKnowledgecenterAPI, getKnowledgeCenter } from "../../../APISearvice/KnowledgeCenterService";
import "../components/knowledgeCenter.scss"
...
import { IKnowledgeCenterMasterProps } from './IKnowledgeCenterMasterProps';
...
const MediaMastercontext = ({ props }: any) => {
  ...
  const ApiCall = async () => {
    ...
    if (groupTitles.includes("intranetadmin")) {
      mediaArr = await getKnowledgeCenter(sp, "yes");
    }
    else if (groupTitles.includes("intranetcontentcontributor")) {
      mediaArr = await getKnowledgeCenter(sp, "No");
    }
  ...
  const Breadcrumb = [
    ...
    {
      "ChildComponent": "Knowledge Center Master",
        ...
        <div className="content" style={{ marginLeft: `${!useHide ? '240px' : '80px'}` }}>
          <div style={{paddingTop:'20px'}} className="container-fluid  paddb">
            ...
            <div className="row pt-0 align-items-center">
              ...
              <a onClick={() => GotoAdd(`${siteUrl}/SitePages/KnowledgeCenterForm.aspx`)}>
...
const KnowledgeCenterMaster: React.FC<IKnowledgeCenterMasterProps> = (props) => (
  <Provider>
    <MediaMastercontext props={props} />
  </Provider>
);

export default KnowledgeCenterMaster;